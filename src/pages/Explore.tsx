...
export default function Explore() {
  ...
  useEffect(() => {
    ...
    async function loadPlaces() {
      setIsLoading(true);
      try {
        // Only resolve current city name when NOT searching a specific location
        if (!query) {
          const name = await getCityName(userLocation!);
          setCityName(name);
        } else {
          // Show the search term as city name immediately (will be overridden by resolvedCity if it's a location)
