  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setTrendingPlaces(places.sort((a, b) => b.rating - a.rating).slice(0, 3));
      } catch (err) {
        console.error(err);
      } finally {
        setIsLoading(false);
    ...
    <div className="min-h-screen">
      <section className="relative pt-20 pb-32 flex flex-col items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 w-full max-w-4xl px-4 flex flex-col items-center text-center">
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-5xl md:text-6xl font-extrabold tracking-tight mb-6">
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-lg md:text-xl text-muted-foreground mb-10 max-w-2xl">
            Discover nearby cafes, hidden gems, and the best hangout spots based on your mood and time of day.
          </motion.p>

          <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }} onSubmit={handleSearch} className="w-full max-w-2xl relative">
            <div className="relative flex items-center w-full h-14 rounded-full bg-card shadow-lg border border-border focus-within:ring-2 focus-within:ring-primary focus-within:border-transparent transition-all overflow-hidden">
              <div className="pl-6 text-muted-foreground"><Search className="w-5 h-5" /></div>
              <input type="text" placeholder="Search for a city, place, or category..." className="w-full h-full bg-transparent border-none focus:outline-none px-4 text-foreground placeholder:text-muted-foreground" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
              <button type="submit" className="h-10 px-6 mr-2 bg-primary hover:bg-primary/90 text-primary-foreground font-medium rounded-full transition-colors">Search</button>
            </div>
          </motion.form>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        {permissionDenied ? (
          <div className="text-center py-10 bg-card border border-border rounded-2xl max-w-xl mx-auto p-8 shadow-sm flex flex-col items-center">
            ...
            <p className="text-sm text-muted-foreground/80 mb-6">Please allow location permissions in your browser settings.</p>
          </div>
