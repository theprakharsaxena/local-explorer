...
function mapGoogleToPlace(gPlace: any): Place {
  ...
  // Photo URLs go directly to Google (not a JSON fetch, browser handles redirects)
  let image = undefined;
  ...
  const distance = Number((Math.random() * 3 + 1).toFixed(1));
...
export async function searchPlaces(query: string = "", ll: string = "28.6139,77.2090", limit: number = 100) {
  ...
  try {
    ...
    if (query && query !== "cafe, restaurant, park, lounge, club") {
      const textUrl = `${PLACE_API}/textsearch/json?query=${encodeURIComponent(query)}&location=${lat},${lng}&radius=10000&key=${API_KEY}`;
      const response = await fetch(textUrl);
      if (!response.ok) return { places: [], isLocationSearch: false };
      ...
      if (data.status === "OK" && data.results.length > 0) {
        ...
        if (isLocation) {
          ...
          const promises = categoriesToFetch.map(async (cat) => {
            const url = `${PLACE_API}/textsearch/json?query=${encodeURIComponent(`${cat} in ${cityName}`)}&key=${API_KEY}`;
    ...
    const promises = categoriesToFetch.map(async (type) => {
      const url = `${PLACE_API}/nearbysearch/json?location=${lat},${lng}&radius=10000&type=${type}&key=${API_KEY}`;
      const res = await fetch(url);
      if (res.ok) {
        const resData = await res.json();
        return resData.results || [];
...
export async function getPlaceDetails(id: string) {
  ...
  try {
    const url = `${PLACE_API}/details/json?place_id=${id}&key=${API_KEY}`;
...
export async function getCityName(ll: string) {
  ...
  try {
    ...
    const placesUrl = `${PLACE_API}/nearbysearch/json?location=${lat},${lng}&radius=10000&type=locality&key=${API_KEY}`;
    ...
    const url = `${GEOCODE_API}/json?latlng=${lat},${lng}&key=${API_KEY}`;
    const response = await fetch(url);
    if (response.ok) {
      const data = await response.json();
