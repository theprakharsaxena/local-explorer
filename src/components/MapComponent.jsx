...
const createCustomMarker = (place) => L.divIcon({
...
export default function MapComponent({ places, userLocation }) {
  ...
  const userCoords = userLocation ? userLocation.split(',').map(Number) : null;
  const center = userCoords || (places.length > 0 ? [places[0].coordinates.lat, places[0].coordinates.lng] : defaultCenter);
      ...
      <MapContainer center={center} zoom={13} style={{ height: '100%', width: '100%' }}>
        ...
        {userCoords && (
          <Marker position={userCoords} icon={userIcon}>
            <Popup><div className="text-center text-xs font-semibold p-1">You are here 📍</div></Popup>
          </Marker>
        )}

        {places.map((place) => (
          <Marker key={place.id} position={[place.coordinates.lat, place.coordinates.lng]} icon={createCustomMarker(place)}>
            <Popup>
              <div className="text-center w-40">
                ...
                <Link to={`/place/${place.id}`} className="block w-full bg-primary text-primary-foreground text-xs py-1 rounded-md text-center hover:bg-primary/90 transition-colors">
                  View Details
                </Link>
              </div>
            </Popup>
